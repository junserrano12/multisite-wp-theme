<?php
	...
	{	
		...
		foreach ($hotel_address as $key => $hotel_address_info) {
			...
			{	
				...
				/* show if display address after h1 is uncheck */
				if( $is_contact_info ){
	...
	if( isset( $hotel_contact ) )
	{	
		$hotel_contact1 = (array)$hotel_contact;
		
		if( array_key_exists( 0, $hotel_contact1 ) ){
		...
		foreach ( $hotel_contact1 as $key => $hotel_contact_info) {
	...
	/* Ouput */
	$html_start = '<span class="vcard">';
	$html_end = '</span>';

	echo $html_start;
	if( isset( $htmladdress ) && $htmladdress ) echo $htmladdress;

	if( isset( $htmlcontact ) && $htmlcontact ) echo $htmlcontact;
	echo $html_end;
