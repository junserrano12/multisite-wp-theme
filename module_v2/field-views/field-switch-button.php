...
<div id="<?php echo $id; ?>-container" class="<?php echo $class; ?>">
    ...
    <div class="field-content-container checkbox-field-container<?php echo $accordioncontent; ?>">
        ...
        <input id="<?php echo $id; ?>-controller" type="checkbox" class="checkbox-switch field-input<?php echo $inputclass; ?>" onchange="_this = jQuery( this ); if( _this.is(':checked') ) { _this.attr('checked', true); _this.prev().val('1') } else { _this.prev().val('0'); _this.attr('checked', false ); }"<?php echo $checked; ?>/>
        <?php if ( $label ) { ?>
        <label for="<?php echo $id; ?>-controller" class="field-title"><?php echo $label; ?></label>
        <?php } ?>
        <p class="label"><?php echo $label; ?></p>
        <input id="<?php echo $id; ?>-button" type="button" class="checkbox-switch-button button" value="Update"/>
        <?php if ( $description ) { ?>
